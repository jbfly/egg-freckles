...
- **2026-08-11 — EF26 is the physical-MP2000 candidate; hardware remains human-gated.** Starting from final EF25, EF26 removes only the synchronous primary-connect diagnostic and restores the final EF13 hardware-proven completion form: `{async: true, reqTimeout: 45000}`. Fresh identity `EggFrecklesEF26:jbfly`, user-visible `1.0-ef26`, package version 40. The server picker, HS/progress path, framing, package tools, ink/Notes, scrolling, and UI are unchanged. Two normalized 114,704-byte builds match at SHA-256 `bcc36db8db643a1e9e1825699a52ffad9bf705617a4af97bed59641f5736b14f`; 61 focused and 140 full tests pass. A seeded disposable Einstein passed exactly one marker/HELLO/READY/MSG/progress/reply connection and was removed. No physical hardware success is claimed. Evidence and the one-session gate order: `runtime/evidence/ef26-physical-candidate/README.md`, `docs/ef26-physical-mp2000-runbook.md`.
