...
| **Case-insensitive symbol collision** | A slot `transcriptTail` made `:TranscriptTail()` call a number (`-48200`). Seen twice: in Track F2 a slot `inkOpen` shadowed the method `InkOpen`, so every ink Send threw `-48200` before reaching the wire. | Slots are `capBytes` / `tailBytes`; the F2 slot was deleted rather than renamed. Grep new slot names against method names. | same; `runtime/evidence/f2round-round.txt`, `f2round-14-ink-thinking.png` |
