...
| **Case-insensitive symbol collision** | A slot `transcriptTail` made `:TranscriptTail()` call a number (`-48200`). Seen twice: in Track F2 a slot `inkOpen` shadowed the method `InkOpen`, so every ink Send threw `-48200` before reaching the wire. | Slots are `capBytes` / `rowChars` / `visibleRows` / `scrollRow` (`tailBytes` went with `TranscriptTail` in Track A8); the F2 slot was deleted rather than renamed. Grep new slot names against method names. | same; `runtime/evidence/f2round-round.txt`, `f2round-14-ink-thinking.png` |
| **Dropping the NIE link to open a second connection** | The Track F2 ink POST first called `:Stop()` (which releases the link) and immediately re-grabbed one; `Bind` succeeded and `connect` then failed `-16009`, *"Phone connection was cut off, or invalid call when not connected"*. One rebuild. | Two endpoints on **one** link. If a link is already held, instantiate the second endpoint with the same `ilid` and skip `InetGrabLink` entirely. | `refs/NewtonProgrammerRef20.txt:73102`; `examples/harness-client/Main.newt` (`InkSend` → `if self.linkID then return :InkOpen();`); `runtime/evidence/f2round-06-ink-sending.png` |
