...
- `examples/harness-client/Main.newt` is the prepared **Egg Freckles 1.0-ef23** with fresh package identity `EggFrecklesEF23:jbfly` (package version 37). M2 retains trunk's no-Dock `pkg_install` / `pkg_remove`, native scroll callbacks, Notes/tools integration, and radio lifecycle, while adding the persisted Advanced server picker, minimized HS-A/HS-B/HS-C handshake probe, and transient `STAT PROGRESS` status painting. The progress branch never changes `responseText`; final `TEXT`/`PROMPT` semantics are unchanged. Two normalized builds are byte-identical at 114,704 bytes with SHA-256 `093d7784c8d097646cfdd1e7cb7b38cb68ef23e8330cc0fd6af9fc5b3cbe6d53`; 139 tests and the isolated `m2ef23-0809a` install/launch/LAN-picker/progress loop are recorded in `runtime/evidence/m2-ef23-integration/`. Physical MP2000 validation remains human-gated. The iPad M4 one-Send probe is complete under the human's photo waiver; see `docs/ef22-server-autofind.md`.
