...
- `examples/harness-client/Main.newt` is **Egg Freckles 1.0-ef13** with the package identity `EggFrecklesEF13:jbfly` (package version 25; the Extras icon reads "Egg Freckles" and, since EF5, draws a 20x14 egg with freckles built by `MakeBinaryFromHex` at tntk time). It supersedes Newton Chat 2.4-a9 / `HarnessClientA9:jbfly`. Since Track L1 it is also the *only* client package: the fixed-op tools client `examples/harness-tools/` was deleted and its POLL long-poll transport and all eight ops (ping, front_app, battery, store_info, pkg_list, note_list, get_note, note_probe) moved verbatim into this file under `Tool*` names (`ToolStart`, `ToolPoll`, `ToolDispatch`, `ToolStop`, ~`Main.newt:1350-1700`), so nothing separate has to be installed. It holds one framed TCP session to port 6801, sends ASCII prompts — one `MSG` frame up to 227 characters, `MSGP` parts above that up to 8 KiB (ROADMAP Track F1) — and renders the model response in a 6 KiB transcript. Since Track F2 it is also the harness panel, and since Track A9 the note path is one **Ask** button meaning *send the newest note, whatever kind it is*: text goes down that same chat transport, any drawing goes out as `NSI1` strokes in a `POST /ink` on 18081 over the *same* NIE link, and a page holding both goes out as one request with the text on an optional `H` line. The reading is appended to the transcript. `Save Note` writes the last reply back as a native note. Since Track A9 there is **no ink capture canvas** — that overlay was deleted, multi-stroke defect and all; drawing happens in stock Notes. Since Track A8 the transcript is wrapped onto a 12-row grid and the **Up**/**Dn** buttons page that window over the whole 6 KiB ring. EF12 is emulator-proven; EF13 has source/unit/build evidence only and its emulator prove is pending. A7 (plus `HarnessToolsR10P`) is what is installed on the physical MP2000, with A3 and A1 as the older fallbacks, and EF13 supersedes A8/A9 and EF1–EF12 so the next install should be Egg Freckles — one package replacing both of the two that are on the device today. **Since EF6 the `/tools` channel is owned by the package-level install-hook agent, not the window**, so it answers with the app closed and restarts itself on every reset; ink is grouped by per-image budgets (`kMaxStrokes` 64, `kMaxPoints` 1600), builds/posts/releases one body at a time, uses in-place string growth, retries oversized bodies at half budget, reports any prefix-only send honestly, and re-arms the watchdog per part; and every NIE-invoked callback is wrapped in `try ... onexception` with one 5-second retry on a bind failure.
