...
- `examples/harness-client/Main.newt` is **Egg Freckles 1.0-ef15** with package identity `EggFrecklesEF15:jbfly` (package version 27). EF15 keeps EF14's radio-off-until-send lifecycle and EF13's concurrent, one-body-at-a-time ink path, opens the chat as a full-screen `protoApp`, removes the redundant Ask/Save panel buttons, and retains the proven custom Up/Dn transcript scroller. During ink sends it keeps `Sending page n/total` visible until the publisher's final close-delimited response streams fixed `STATUS` lines; `INK` and `INKERR` remain the only final results (`examples/harness-client/Main.newt:10-11,346-355,2014-2027,2155-2230`; `pkg_publisher.py:458-490,628-640`). The first chat or ink send starts `/tools` on the same Newton-initiated link; a ticketed five-second idle callback then stops chat, ink, tools, and releases NIE. The accepted consequence is that the host cannot push while the Newton is idle. EF12 remains the last emulator-proven build; EF13-EF15 have source/unit/build evidence only, and physical MP2000 validation is pending. The package still carries the two stock Notes actions, a 6 KiB transcript, and the eight fixed `/tools` operations.
