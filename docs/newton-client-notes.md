...
- `examples/harness-client/Main.newt` is **Egg Freckles 1.0-ef14** with package identity `EggFrecklesEF14:jbfly` (package version 26). EF14 preserves EF13's streamed, one-body-at-a-time ink path and changes only radio lifecycle: install/reset and window open do not acquire NIE; the first chat or ink send starts the `/tools` poll on the same Newton-initiated link; the link stays up through the reply and a ticketed five-second idle callback then stops chat, ink, tools, and releases NIE (`examples/harness-client/Main.newt:554-561,746-770,835-876,2040-2096,2300-2304,2590-2608`). The accepted consequence is that the host cannot push while the Newton is idle. EF12 remains the last emulator-proven build; EF13 and EF14 have source/unit/build evidence only, and physical MP2000 validation is pending. The rest of the merged client remains unchanged: framed chat on 6801, streamed `NSI1` ink on 18081, the two stock Notes actions, a 6 KiB transcript, and the eight fixed `/tools` operations.
