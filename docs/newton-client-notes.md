...
- `examples/harness-client/Main.newt` is the prepared **Egg Freckles 1.0-ef24** with fresh package identity `EggFrecklesEF24:jbfly` (package version 38). It is the smallest M4 follow-up hypothesis: M2 changed only the primary chat connect `reqTimeout` from 45,000 ms to 10,000 ms, and EF24 restores it to 45,000 ms while leaving the 12-second post-connect handshake watchdog and marker output's 10,000 ms request timeout unchanged. Two normalized builds are byte-identical at 114,704 bytes with SHA-256 `5147937cd38086aa2b5ac258630f7f51f03e04d246d41fe3c440cd8a735981ba`; 45 focused and 140 full tests pass, and a disposable seeded Einstein completed the local marker/HELLO/READY handshake. Evidence is `runtime/evidence/ef24-chat-timeout/README.md`. This is Linux emulator evidence, not iOS or physical-hardware proof; EF23's M4 observation remains the only iOS result.
