...
- `examples/harness-client/Main.newt` is **Egg Freckles 1.0-ef18** with package identity `EggFrecklesEF18:jbfly` (package version 30). EF18 adds the P4 Notes-menu progress float: the persistent note agent opens a root-level `protoFloatNGo` immediately before its existing asynchronous `SendInk`, mirrors the existing sending-page counter and parsed `STATUS` messages through `SetStatus`, shows `Answer filed in AI` or the `INKERR` text briefly, then closes after 1.5 seconds without changing reply filing or EF14 radio ownership (`examples/harness-client/Main.newt:149-179,208-258,306-359`). EF18 also stamps each existing NSI1 header block with `T <build_ms> -1`; the publisher validates it and logs `INKTIME page n/total build X send -1`, making the Newton's per-page build cost visible on Mars. `-1` is deliberate because a request cannot contain its own not-yet-measured send duration; the existing local post/response `Print` remains (`Main.newt:1668-1735,1769-1807`; `pkg_publisher.py:370-440`). No stroke/delta encoding changed; EF19 remains the measured encoder round. EF13 is hardware-proven; EF14-EF18 require the human-gated MP2000 validation.
