...
- `examples/harness-client/Main.newt` is **Egg Freckles 1.0-ef17** with package identity `EggFrecklesEF17:jbfly` (package version 29). EF17 keeps EF16's supported Notes `'entryAdded` refresh, EF15's full-screen progress UI, EF14's radio-off-until-send lifecycle, and EF13's one-body-at-a-time stream. It reverts only EF16's fixed-buffer encoder to the EF15 `Clone`/`StrMunger` encoder: `MakeBinary(..., 'string)` retained its allocated Unicode length instead of terminating at the first untouched zero, so every body was padded to 16,384 characters before `InkPost` and hardware sent no `/ink` request. The small emulator probe `MakeBinary(10, 'string)` plus one `StuffUniChar` returned `StrLen=4`, proving the broken assumption. Per-page `INKTIME` logging remains. EF13 is hardware-proven; EF14-EF17 require the human-gated MP2000 validation (`examples/harness-client/Main.newt:10-11,318-345,1599-1690,2020-2290`; Newton Programmer's Reference: `MakeBinary`, `StuffUniChar`).
