...
- `examples/harness-client/Main.newt` is **Egg Freckles 1.0-ef16** with package identity `EggFrecklesEF16:jbfly` (package version 28). EF16 keeps EF15's full-screen progress UI, EF14's radio-off-until-send lifecycle, and EF13's one-body-at-a-time stream. It adds the supported Notes `'entryAdded` notification after filing a reply and replaces repeatedly grown multipart strings with one fixed 16 KiB Unicode buffer written by `StuffUniChar`. Each page logs `INKTIME page N/T build X ms` and `send X ms` from `Ticks()` so the physical MP2000 can separate encoding from transport. Pagination and the 16 KiB body cap are unchanged. EF13 is hardware-proven; EF14-EF16 have source/unit/build evidence and await the human-gated MP2000 validation. The package still carries the two stock Notes actions, a 6 KiB transcript, and the eight fixed `/tools` operations (`examples/harness-client/Main.newt:10-11,318-345,1599-1725,2035-2290`).
