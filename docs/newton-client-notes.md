...
- `examples/harness-client/Main.newt` is **Egg Freckles 1.0-ef24** with fresh package identity `EggFrecklesEF24:jbfly` (package version 38). M2 changed only the primary chat connect `reqTimeout` from 45,000 ms to 10,000 ms, and EF24 restores it to 45,000 ms while leaving the 12-second post-connect handshake watchdog and marker output's 10,000 ms request timeout unchanged. Two normalized builds are byte-identical at 114,704 bytes with SHA-256 `5147937cd38086aa2b5ac258630f7f51f03e04d246d41fe3c440cd8a735981ba`; 45 focused and 140 full tests pass, and a disposable seeded Einstein completed the local marker/HELLO/READY handshake (`runtime/evidence/ef24-chat-timeout/README.md`). The first physical iPad run visibly changed EF23's `Connect error -16005` result to `Connect error -16013`, but reached no HS-A/B/C stage or reply; its pcap missed the Send, while the sanitized Mars journal records seven accepts and no protocol/application bytes. Hardware proof failed (`runtime/evidence/ef24-ipad-physical-20260811/README.md:9-38`; `runtime/evidence/ef24-ipad-physical-20260811/mars-journal-summary.txt:7-14`).
