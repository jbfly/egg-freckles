...
- `examples/harness-client/Main.newt` is the prepared **Egg Freckles 1.0-ef22** with fresh package identity `EggFrecklesEF22:jbfly` (package version 34). EF22 adds the no-Dock `pkg_install` / `pkg_remove` operations on top of EF21’s full-screen native scroll callbacks; isolated emulator acceptance installed/launched it, moved `scrollRow` 0→10→0 with the ROM arrows, and completed package install/remove over the tools broker (`docs/mars-deploy-devloop.md`; `runtime/evidence/ef22-validation/`). EF21 remains the production fallback until the human-gated mars/Newton steps pass. EF20’s radio teardown and transport timing remain unchanged. EF20 fixes the Notes completion path so the terminal `INK` reply enters delayed teardown directly, and treats `TOOLS 0 ping` as a heartbeat rather than activity; the isolated emulator completed eight `/ink` POSTs and left no endpoint/socket after idle. EF20 also reports Newton output-completion and endpoint dispose+reopen timing through the existing tools heartbeat without retaining ink bodies or changing the encoder. Emulator medians were 8 ms send and 99 ms intermediate endpoint churn while POST spacing remained 6-7 s, so no speculative transport rewrite ships; physical MP2000 timing remains human-gated (`docs/ef20-radio-transport.md`; `runtime/evidence/ef20-final-publisher.log`). EF19's required three-argument `StrPos(body, marker, 0)`, EF13 memory bounds, callback armor, endpoint-presence guard, and one bind retry remain unchanged.
