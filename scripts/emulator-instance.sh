...
SUBPROCESS_TIMEOUT=${EMULATOR_SUBPROCESS_TIMEOUT:-60}

bounded() { timeout -k 5 "$SUBPROCESS_TIMEOUT" "$@"; }
command=${1:-}
instance=${2:-}
...
main_env=
if [ ! -f .env ]; then
	common=$(bounded git rev-parse --git-common-dir 2>/dev/null || true)
...
if [ -n "$main_env" ]; then
	compose() { bounded "$COMPOSE" --env-file "$main_env" "$@"; }
else
	compose() { bounded "$COMPOSE" "$@"; }
fi
...
free_port() {
	bounded python3 -c 'import socket
s = socket.socket()
s.bind(("127.0.0.1", 0))
...
up)
	...
	container=$(bounded podman ps -a --filter "label=com.docker.compose.project=$name" \
	...
	printf 'instance   %s\n' "$instance"
	printf 'container  %s\n' "$container"
	printf 'control    http://127.0.0.1:%s\n' "$(bounded podman port "$container" 8080 | cut -d: -f2)"
	printf 'novnc      http://127.0.0.1:%s/vnc.html?autoconnect=1\n' \
		"$(bounded podman port "$container" 6080 | cut -d: -f2)"
	;;
down)
	name=$(project)
...
list)
	...
	bounded podman ps --filter name=_emulator_1 \
