...
| `harness-client` | `HarnessClientA7:jbfly` ("Chat A7", v2.4-a7) | CURRENT chat client and **the harness panel**: chat + `Ask Note` / `Save Note` + an `Ink` overlay, all in one app (ROADMAP Track F2). Long prompts and long notes split into `MSGP` parts (Track F1); built by `make newton-packages`. `HarnessClientA3` is what is installed on the physical MP2000 — everything since is emulator-proven only |
