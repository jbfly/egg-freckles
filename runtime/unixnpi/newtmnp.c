...
int InitNewtDev(NewtDevInfo *newtDevInfo)
{
	...
	if((newtFd = open(newtDevInfo->devName, O_RDWR | O_NOCTTY)) == -1) {
		perror(newtDevInfo->devName);
		return -1;
	}

	/* Get the current device settings */
	if(tcgetattr(newtFd, &newtTty) < 0) {
		perror("tcgetattr");
		close(newtFd);
		return -1;
	}
	...
	if(tcsetattr(newtFd, TCSANOW, &newtTty) < 0) {
		perror("tcsetattr");
		close(newtFd);
		return -1;
	}
...
void SendFrame(int newtFd, const uchar *head, const uchar *info, int infoLen)
{
...
void SendLTFrame(int newtFd, uchar seqNo, const uchar *info, int infoLen)
{
	uchar ltFrameHead[3] = {
